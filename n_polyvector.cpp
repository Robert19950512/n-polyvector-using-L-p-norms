...
namespace igl {
	...
	public:
		...
		IGL_INLINE bool solve(const Eigen::VectorXi &isConstrained,
			...
			Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> &Eout,
			double p);
...
//		std::cout << "this is the function we want to implement" << std::endl;
	...
	double B;
	int ENum = 0;
	Eout.resize(numInteriorEdges, 3);
	Eout.setConstant(0);
	Eigen::VectorXd debuge(numE);
	debuge.setConstant(-5);
	for (unsigned eid = 0; eid < numE; eid++)
	{
		if (!isBorderEdge[eid])
		{
			if (p != 2) {
				...
				//				std::cout << "x0" << x[fid0]  << "x1" << x[fid1] << std::endl;
				//				std::cout << "x" << x << std::endl;
				//				std::cout << "n" << n << std::endl;
				//				std::cout << "A" << A << std::endl;
				//				std::cout << "rotation complex in cw" << std::polar(1., 1.*n*K[eid]) << std::endl;
				B = pow(abs(imag(A)), 2 - p) + pow(abs(real(A)), 2 - p);
				W[eid] = 1 / (pow(B, (1 / (2 - p))) + 0.0001);
				...
				debuge[eid] = e;
				Eout(ENum, 0) = EV(eid, 0);
				Eout(ENum, 1) = EV(eid, 1);
				Eout(ENum, 2) = e;
				ENum++;
			}
			else {
	...
	esum = 0;
	//	std::cout << "debuge" << debuge << std::endl;
}
	...
	Eigen::Matrix<std::complex<typename DerivedV::Scalar>, Eigen::Dynamic, 1> &x)
{
	...
	//  std::cout << "isconstraint" << isConstrained << std::endl;
	//  std::cout << "xknown" << xknown << std::endl;
...
solve(const Eigen::VectorXi &isConstrained,
	const Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> &cfW,
	Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> &output,
	Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> &E,
	double po)
{
	...
	for (unsigned eid = 0; eid<numE; ++eid)
	{
		if (!isBorderEdge[eid])
		{
			...
			tripletList.push_back(Eigen::Triplet<std::complex<typename DerivedV::Scalar> >(fid1,
				fid0,
				-1.*W[eid] * std::polar(1., 1.*n*K[eid])));
			//	  std::cout << "rotation complex in cpcl" << std::polar(1., 1.*n*K[eid]) << std::endl;
		}
	}
	D.resize(numF, numF);
	...
	double po)
{
	...
	pvff.solve(isConstrained, cfW, output, Eout, po);
